...
Ext.onReady(function(){
	...
	var vGridPanel= new Ext.ux.GridPanel({
        ...
        cm: new Ext.grid.ColumnModel({
        	...
        	columns:[ new Ext.grid.RowNumberer(),
        	          ...
        	          {header:'应收单号',width:140,dataIndex:'vcNo',renderer:function(a,b,c){return c.data['fidel']=='1'?'已作废/'+a:a; }},
        	          ...
        	          {header:'单据来源',width:80,dataIndex:'sourceType',renderer:function(a){return a=='1'?'销退':'销售'}},
        	          {header:'销售/退单号',width:120,dataIndex:'sourceNo'},
        	          ...
        	          {header:'应收金额',width:100,dataIndex:'dlMoney'},
	...
	var vToobar1=new Ext.Toolbar({
	    items: ['业务日期：',vStartDate,'到',vEndDate,'-','客户：',vKhCombo1,'-','结算方式：',vSettlementCombo1]
	});
	
	/**工具栏2**/
	var vToobar2=new Ext.Toolbar({
		items:['配送方式：',vLogisticsCombo1,'-','物流公司：',vWlCombo,'-','-','结算状态：',vJsState1,
		       '-',{
        	xtype:'textfield',
        	id:'keyWord',
        	maxLength:100,
...
//	    		}});
	...
	function funcEdit(type,row){
		...
		var vsourceNo=new Ext.form.TextField({name:'sourceNo',fieldLabel:'销售单号',width:125,allowBlank:false,readOnly:true,style:"background:#F6F6F6"});
...
//    							addWindow.show();
    			...
    			handler : function() {
    				if (addForm1.getForm().isValid()) {
    					...
    					if(!record){
    						...
    						panel.findField('sourceType').setValue(record.get('vcType')=='销售'?0:1);
    						panel.findField('khId').setValue(record.get('khId'));
    						panel.findField('dtBs').setValue(new Date(parseInt(record.get("dtBs"))));
    						panel.findField('settlementId').setValue(record.get("iSettlement")=='0'?'':record.get("iSettlement"));
    						panel.findField('logisticsId').setValue(record.get("iLogistics")=='0'?0:record.get("iLogistics"));
    						panel.findField('dlMoney').setValue(record.get('vcType')=='销售'?record.get('dlMoney'):-record.get('dlMoney'));
    						addWindow1.close();
    					}
    				}
    			}
	...
	function funcinitgrid(){
		...
		vGridStore.setBaseParam('iwl',vWlCombo.getValue());
		vGridStore.load({params:{
			start:0, 
			limit:20
		}});
