...
Ext.onReady(function(){
    ...
    var grid = new Ext.ux.GridPanel({
        ...
        cm: new Ext.grid.ColumnModel({
			...
			columns:[new Ext.grid.RowNumberer(),
			    {header: '账目类型编号',width:100, align:'center',dataIndex:'vcNo'},
			    {header: '账目类型名称', width: 150, align:'center',dataIndex: 'vcName'},
			    {header: '收支类型',width:100,align:'center',dataIndex:'itype',renderer:function(a){return a=='0'?'支出':'收入';}},
        ...
        tbar:[{
        	...
        	handler: function(){
        		var record= grid.getSelectionModel().getSelected();
				if(!record){
                	Ext.Msg.alert('信息提示','请选择要删除的账目类型');  
				}else{
					if(record.get('vcRemark').indexOf('系统必须')>-1){
						Ext.Msg.alert('信息提示','系统必须,不能删除');
						return;
					}
					Ext.MessageBox.confirm('删除提示', '是否删除该账目类型？', function(c) {
					   if(c=='yes'){
					   		Ext.Ajax.request({
					   			url : "ut_delete.do?type=zmlx",
    ...
    function funcEdit(type,gridrow){
    	...
    	var vType=new Ext.form.Hidden({name:'type',value:'zmlx'});
    	var vVcNo= new Ext.form.TextField({name:'vcNo',fieldLabel:'账目类型编号',width:260,maxLength:10,allowBlank: false});
    	var vVcName= new Ext.form.TextField({name:'vcName',fieldLabel:'账目类型名称',width:260,maxLength:25,allowBlank: false});
