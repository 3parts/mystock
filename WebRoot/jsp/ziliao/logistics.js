...
Ext.onReady(function(){
    ...
    var grid = new Ext.ux.GridPanel({
        ...
        cm: new Ext.grid.ColumnModel({
			...
			columns:[new Ext.grid.RowNumberer(),
			    {header: '配送方式编号',width:100, align:'center',dataIndex:'vcNo'},
			    {header: '配送方式名称', width: 150, align:'center',dataIndex: 'vcName'},
        ...
        tbar:[{
        	...
        	handler: function(){
        		var record= grid.getSelectionModel().getSelected();
				if(!record){
                	Ext.Msg.alert('信息提示','请选择要删除的配送方式');  
				}else{
					Ext.MessageBox.confirm('删除提示', '是否删除该配送方式？', function(c) {
					   if(c=='yes'){
					   		Ext.Ajax.request({
					   			url : "ut_delete.do?type=psfs",
    ...
    function funcEdit(type,gridrow){
    	...
    	var vType=new Ext.form.Hidden({name:'type',value:'psfs'});
    	var vVcNo= new Ext.form.TextField({name:'vcNo',fieldLabel:'配送方式编号',width:260,maxLength:10,allowBlank: false});
    	var vVcName= new Ext.form.TextField({name:'vcName',fieldLabel:'配送方式名称',width:260,maxLength:25,allowBlank: false});
