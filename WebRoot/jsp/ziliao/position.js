...
Ext.onReady(function(){
    ...
    var grid = new Ext.ux.GridPanel({
        ...
        cm: new Ext.grid.ColumnModel({
			...
			columns:[new Ext.grid.RowNumberer(),
			    {header: '职位编号',width:100, align:'center',dataIndex:'vcNo'},
			    {header: '职位名称', width: 150, align:'center',dataIndex: 'vcName'},
        ...
        tbar:[{
        	...
        	handler: function(){
        		var record= grid.getSelectionModel().getSelected();
				if(!record){
                	Ext.Msg.alert('信息提示','请选择要删除的职位');  
				}else{
					Ext.MessageBox.confirm('删除提示', '是否删除该职位？', function(c) {
					   if(c=='yes'){
					   		Ext.Ajax.request({
					   			url : "ut_delete.do?type=zw",
    ...
    function funcEdit(type,gridrow){
    	...
    	var vType=new Ext.form.Hidden({name:'type',value:'zw'});
    	var vVcNo= new Ext.form.TextField({name:'vcNo',fieldLabel:'职位编号',width:260,maxLength:10,allowBlank: false});
    	var vVcName= new Ext.form.TextField({name:'vcName',fieldLabel:'职位名称',width:260,maxLength:25,allowBlank: false});
