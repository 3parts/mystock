...
public class SplbBizImpl implements SplbBiz {
	...
	public Integer saveOrUpdateSplb(SplbDTO dto) {
		...
		if (dto.getLbid() != null) {
			...
			splb.setCompanyId(dto.getCompanyId());
		}
	...
	public boolean deleteSplb(Integer lbid) {
		// int count = baseDao
		// .countQuery("select count(*) from Spxx where lbid = " + lbid);
		// if (count > 0) {
		// return false;
		// }
		...
		// return true;
		int count = baseDao
				.countQuery("select count(*) from Tbcommodity where typeId="
						+ lbid);
		if (count > 0) {
			return false;
		}
	...
	public List findSplbTree() {
		List list = baseDao.findByHql("from Splb t where "
	...
	public List findCombox() {
		...
		for (int i = 0; i < list.size(); i++) {
			splb = new Splb();
			splb = (Splb) list.get(i);
			comb = new ComboData();
			comb.setText(splb.getLbname());
			comb.setValue(splb.getLbid() + "");
			list1.add(comb);
		}
		return list1;
	}
