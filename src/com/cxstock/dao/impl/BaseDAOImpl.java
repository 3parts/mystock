...
public class BaseDAOImpl extends HibernateDaoSupport implements BaseDAO {
	...
	/** 分页装载指定类的查询结果 */
	public List findByProperty(String clazz, String propertyName, String value,
			int start, int limit) {
		String hql = "from " + clazz + " as model where model." + propertyName
				+ "= '" + value + "'";
		return this.findByHql(hql, start, limit);
	...
	public List listAll(String clazz, int start, int limit, String wheres) {
		/* 加载用户独有的 hibernate */
		final int pStart = start;
		final int pLimit = limit;
		final String hql = "from " + clazz + " where " + wheres;
		List list = getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException {
				Query query = session.createQuery(hql);
