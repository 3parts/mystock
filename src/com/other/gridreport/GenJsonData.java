...
import com.other.gridreport.GenDataBase.ReportQueryItem;
import com.other.gridreport.GenDataBase.ResponseDataType;
import com.other.gridreport.GenDataBase.jdbc_param;
...
public class GenJsonData {
	...
	public static void JSON_GenMultiRecordset(HttpServletResponse response,
		...
		try {
			...
			Connection con = DriverManager.getConnection(jdbc_param.url,
					jdbc_param.user, jdbc_param.password); // 用适当的驱动程序类与 DBMS
			// 建立一个连接
			...
			for (ReportQueryItem QueryItem : QueryItems) {
				DoGenOneRecordsetText(JsonText, QueryItem.QuerySQL,
						QueryItem.RecordsetName, stmt, (Count == QueryItems
								.size()));
				++Count;
			}
			JsonText.append('}');
			...
			GenDataBase.ResponseText(response, JsonText.toString(), DataType);
		} catch (Exception e) {
			try {
	...
	public static void JSON_GenMultiRecordset(HttpServletResponse response,
		...
		JSON_GenMultiRecordset(response, QueryItems,
				GenDataBase.DefaultReportDataType);
	}

	public static void JSON_GenOneRecordset(HttpServletResponse response,
	...
	private static void DoGenOneRecordsetText(StringBuffer JsonText,
			String QuerySQL, String RecordsetName, Statement stmt,
			boolean LastRecordset) {
		try {
			...
			while (rs.next()) {
				...
				for (int i = 1; i <= ColCount; i++) {
					...
					if (ColType == Types.LONGVARBINARY
						...
						if (!rs.wasNull())
							JsonText.append((new sun.misc.BASE64Encoder())
									.encode(BinData));
					} else {
	...
	public static void JSON_GenDetailData(HttpServletResponse response,
			String QuerySQL, ResponseDataType DataType) {
	...
	public static void JSON_GenEntireData(HttpServletResponse response,
			String QuerySQL, String ParameterPart, ResponseDataType DataType) {
		try {
			try {
				Class.forName(jdbc_param.driver); // Class.forName 装载驱动程序
				Connection con = DriverManager.getConnection(jdbc_param.url,
						jdbc_param.user, jdbc_param.password); // 用适当的驱动程序类与
				// DBMS 建立一个连接
				Statement stmt = con.createStatement(); // 用于发送简单的SQL语句
				ResultSet rs = stmt.executeQuery(QuerySQL);
				ResultSetMetaData rsmd = rs.getMetaData();
				...
				GenDataBase.ResponseText(response, JsonText.toString(),
						DataType);
			} catch (Exception e) {
				// output error message
				PrintWriter pw = response.getWriter();
	...
	public static void JSON_GenEntireData(HttpServletResponse response,
		...
		JSON_GenEntireData(response, QuerySQL, ParameterPart,
				GenDataBase.DefaultReportDataType);
	}

	// 这里只产生报表参数数据，数据将加载到报表参数、非明细网格中的部件框中
	// 当报表没有明细时，调用本方法生成数据，查询SQL应该只能查询出一条记录
	public static void JSON_GenParameterData(HttpServletResponse response,
			String QuerySQL, ResponseDataType DataType) {
		try {
			StringBuffer JsonText = new StringBuffer("{");
	...
	public static void JSON_GenParameterData(HttpServletResponse response,
			String QuerySQL) {
		JSON_GenParameterData(response, QuerySQL,
				GenDataBase.DefaultReportDataType);
	}
