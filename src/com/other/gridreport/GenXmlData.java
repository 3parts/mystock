...
import com.other.myclass.ReadXml;
...
public class GenXmlData {
	...
	public static void XML_GenMultiRecordset(HttpServletResponse response,
		...
		try {
			...
			Class.forName(rd.getDriver()); // Class.forName 装载驱动程序
			Connection con = DriverManager.getConnection(rd.getUrl(), rd
					.getName(), rd.getPwd()); // 用适当的驱动程序类与 DBMS
			// 建立一个连接
			...
			GenDataBase.ResponseText(response, XmlText.toString(), DataType);
			...
			while (rs.next()) {
				...
				for (int i = 1; i <= ColCount; i++) {
					...
					if (ColType == Types.LONGVARBINARY
						...
						if (!rs.wasNull()) {
							if (Grcommon.HasSpecialChar(Val))
								XmlText.append(Grcommon.HTMLEncode(Val));
							else
								XmlText.append(Val);
						}
					}
	...
	public static void XML_GenEntireData(HttpServletResponse response,
		...
		// ArrayList<ReportQueryItem> QueryItems = new
		// ArrayList<ReportQueryItem>();
